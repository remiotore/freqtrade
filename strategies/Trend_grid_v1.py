...
class Trend_grid_v1(IStrategy):
    ...
    grid_ratio = DecimalParameter(0.01, 0.05, default=0.03, decimals=3, space="buy", optimize=True)
    drawdown_ratio = DecimalParameter(0.01, 0.05, default=0.015, decimals=3, space="sell", optimize=True)
    ...
    def adjust_trade_position(
        ...
        **kwargs,
        ...
        init_grid_price = filled_entries[0].price
        num_filled_entries = len(filled_entries)

        if trade.is_short:
            add_position_price = init_grid_price * (1 - self.grid_ratio.value) ** num_filled_entries
            exit_position_price = init_grid_price * (1 - self.drawdown_ratio.value) ** (num_filled_entries - 1) * (1 + self.grid_ratio.value)
            if current_rate < add_position_price:
                try:
                    stake_amount = filled_entries[-1].stake_amount
                    return stake_amount, "grid_short_increase"
                except Exception as exception:
                    return None
            
            if current_rate > exit_position_price:
                return -trade.stake_amount, "grid_short_exit"
            ...
            if current_rate > add_position_price:
                try:
                    stake_amount = filled_entries[-1].stake_amount
                    return stake_amount, "grid_long_increase"
                except Exception as exception:
                    print(f"Exception: {exception}")
                    return None
            
            if current_rate < exit_position_price:
                return -trade.stake_amount, "grid_long_exit"
