...
class mokuStrat(IStrategy):
    ...
    def custom_exit(self, pair: str, trade: 'Trade', current_time: 'datetime', current_rate: float, current_profit: float, **kwargs):
        ...
        risk = Trade.open_rate - dataframe['stoploss_line']
        target = risk * 2
        risk_reward_ratio = target / risk

        if (last_candle['close'] >= target):
            return "target_reach_sell"
    ...
    def populate_exit_trend(self, dataframe: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        ...
        dataframe.loc[
            (
                ...
                (qtpylib.crossed_below(dataframe['lagging'], dataframe['conversion']))
            ),
            'exit_long'] = 1
        return dataframe
