...
class BestProfitable5MinStrategy(IStrategy):
    """
    ...
    # -------------------------------------------------------------------------
    # 3) Trailing Stop Hyperparameters
    # -------------------------------------------------------------------------
    trailing_stop_opt = CategoricalParameter([True, False], default=False, space='buy', optimize=True)
    trailing_stop_positive = DecimalParameter(0.01, 0.10, default=0.02, decimals=3, space='buy', optimize=True)
    trailing_stop_positive_offset = DecimalParameter(0.01, 0.15, default=0.03, decimals=3, space='buy', optimize=True)
    trailing_only_offset_is_reached = CategoricalParameter([True, False], default=False, space='buy', optimize=True)
    ...
    def bot_start(self, **kwargs) -> None:
        """
        Log final parameter selections when the bot starts.
        """
        super().bot_start(**kwargs)
        logger.info("Final Strategy Parameters:")
        logger.info(f"  timeframe            = {self.timeframe}")
        logger.info(f"  trailing_stop        = {self.trailing_stop_opt.value}")
        if self.trailing_stop_opt.value:
            logger.info(f"    trailing_stop_positive        = {self.trailing_stop_positive.value}")
            logger.info(f"    trailing_stop_positive_offset = {self.trailing_stop_positive_offset.value}")
            logger.info(f"    trailing_only_offset_is_reached = {self.trailing_only_offset_is_reached.value}")
