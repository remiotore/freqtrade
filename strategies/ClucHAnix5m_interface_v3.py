...
class ClucHAnix_BB_RPB_MOD2_ROI_DYNAMIC_TB(ClucHAnix5m_interface_v3):
                ...
                if len(dataframe) >= 1:
                    ...
                    if trailing_buy["allow_trailing"]:
                        if (
                            ...
                            self.custom_info_trail_buy[pair]["trailing_buy"][
                                "enter_tag"
                            ] = f"initial_buy_tag (strat trail price {last_candle['close']})"
                            self.custom_info_trail_buy[pair]["trailing_buy"][
                                "start_trailing_time"
                            ...
                            elif current_price < trailing_buy[
                                "start_trailing_price"
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if self.trailing_buy_order_enabled and self.config["runmode"].value in (
            ...
            last_candle = dataframe.iloc[-1].squeeze()
            logger.info("last_candle: %s", last_candle)
...
class ClucHAnix5m_interface_v3(IStrategy):
    INTERFACE_VERSION = 3
    ...
    pHSL = DecimalParameter(
        -0.5, -0.04, default=-0.08, decimals=3, space="sell", load=True
    )
    ...
    lambo1_rsi_4_limit = IntParameter(
        5, 60, default=buy_params["lambo1_rsi_4_limit"], space="buy", optimize=True
    )
    lambo1_rsi_14_limit = IntParameter(
        5, 60, default=buy_params["lambo1_rsi_14_limit"], space="buy", optimize=True
    )
    ...
    local_trend_ema_diff = DecimalParameter(
        0, 0.2, default=buy_params["local_trend_ema_diff"], space="buy", optimize=True
    )
    ...
    ewo_candles_buy = IntParameter(
        2, 30, default=buy_params["ewo_candles_buy"], space="buy", optimize=True
    )
    ewo_candles_sell = IntParameter(
        2, 35, default=buy_params["ewo_candles_sell"], space="buy", optimize=True
    )
    ...
    ewo_high = DecimalParameter(
        2.0, 15.0, default=buy_params["ewo_high"], space="buy", optimize=True
    )
    ewo_1_rsi_14 = IntParameter(
        10, 100, default=buy_params["ewo_1_rsi_14"], space="buy", optimize=True
    )
    ewo_1_rsi_4 = IntParameter(
        1, 50, default=buy_params["ewo_1_rsi_4"], space="buy", optimize=True
    )
    ewo_low_rsi_4 = IntParameter(
        1, 50, default=buy_params["ewo_low_rsi_4"], space="buy", optimize=True
    )
    ...
    cofi_ema = DecimalParameter(
        0.6, 1.4, default=buy_params["cofi_ema"], space="buy", optimize=True
    )
    cofi_fastk = IntParameter(
        1, 100, default=buy_params["cofi_fastk"], space="buy", optimize=True
    )
    cofi_fastd = IntParameter(
        1, 100, default=buy_params["cofi_fastd"], space="buy", optimize=True
    )
    cofi_adx = IntParameter(
        1, 100, default=buy_params["cofi_adx"], space="buy", optimize=True
    )
    cofi_ewo_high = DecimalParameter(
        1.0, 15.0, default=buy_params["cofi_ewo_high"], space="buy", optimize=True
    )

    nfi32_rsi_4 = IntParameter(
        1, 100, default=buy_params["nfi32_rsi_4"], space="buy", optimize=True
    )
    ...
    ewo_1_enabled = BooleanParameter(
        default=buy_params["ewo_1_enabled"], space="buy", optimize=True
    )
    ewo_low_enabled = BooleanParameter(
        default=buy_params["ewo_low_enabled"], space="buy", optimize=True
    )
    cofi_enabled = BooleanParameter(
        default=buy_params["cofi_enabled"], space="buy", optimize=True
    )
    lambo1_enabled = BooleanParameter(
        default=buy_params["lambo1_enabled"], space="buy", optimize=True
    )
    lambo2_enabled = BooleanParameter(
        default=buy_params["lambo2_enabled"], space="buy", optimize=True
    )
    local_trend_enabled = BooleanParameter(
        default=buy_params["local_trend_enabled"], space="buy", optimize=True
    )
    nfi32_enabled = BooleanParameter(
        default=buy_params["nfi32_enabled"], space="buy", optimize=True
    )
    clucha_enabled = BooleanParameter(
        default=buy_params["clucha_enabled"], space="buy", optimize=True
    )
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe["btc_1m"] = self.dp.get_pair_dataframe("BTC/USDT", timeframe="1m")[
            "close"
        ]
        btc_1d = (
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[
            is_btc_safe & is_pump_safe & reduce(lambda x, y: x | y, conditions),
            "enter_long",
        ] = 1
    ...
    def confirm_trade_exit(
        ...
        **kwargs,
        ...
        if type(trade.enter_tag) == str:
            trade.exit_reason = exit_reason + "_" + str(trade.enter_tag)
        else:
