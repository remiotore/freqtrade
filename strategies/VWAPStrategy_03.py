...
class VWAPStrategy_03(IStrategy):
    ...
    def custom_stoploss(self, pair: str, trade: 'Trade', current_time: datetime, current_rate: float, current_profit: float, after_fill: bool, **kwargs) -> float:
        ...
        if trade.id in self.custom_info:
            ...
            if current_profit >= 0.02:
                devided_current_profit = current_profit / 2

                if devided_current_profit < self.custom_info[trade.id]:
                    self.custom_info[trade.id] = devided_current_profit
        ...
        if pd.notna(last_candle['ATR_stoploss']):
            self.custom_info[trade.id] = last_candle['ATR_stoploss']
            return stoploss_from_absolute(last_candle['ATR_stoploss'])
    ...
    def populate_entry_trend(self, dataframe: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        ...
        dataframe.loc[
            (
                ...
                (dataframe['close'].shift(1) <= dataframe['lower_band'].shift(1)) & # Buy when the previous closing price was less than or equal to the previous lower bband
    ...
    def populate_exit_trend(self, dataframe: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        ...
        dataframe.loc[
            (
                ...
                (dataframe['close'] >= dataframe['upper_band']) & # Sell when closing price is the same or more than upper bband
                (dataframe['rsi'] > 70) & # Sell when rsi > 55
                (dataframe['lower_band'] != dataframe['upper_band']) # Make sure lower and upper bband is not the same (no momentum)
            ),
