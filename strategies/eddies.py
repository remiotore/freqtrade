...
class eddies(IStrategy):
    ...
    # Stoploss:
    stoploss = -0.25  # edit by Mastaaa, Original -0.3

    # Trailing stop:
    trailing_stop = False  # value loaded from strategy
    trailing_stop_positive = 0.002  # value loaded from strategy
    ...
    # Buy Params
    base_nb_candles_buy = IntParameter(5, 30, default=buy_params['base_nb_candles_buy'], space='buy', optimize=True)
    lookback_candles = IntParameter(2, 40, default=buy_params['lookback_candles'], space='buy', optimize=True)
    profit_threshold = DecimalParameter(0.95, 1.09, default=buy_params['profit_threshold'], space='buy', optimize=True)

    low_offset = DecimalParameter(0.5, 0.95, default=buy_params['low_offset'], space='buy', optimize=True)
    low_offset_2 = DecimalParameter(0.7, 0.95, default=buy_params['low_offset_2'], space='buy', optimize=True)
    ...
    buy_sma15_32 = DecimalParameter(0.85, 0.95, default=buy_params['buy_sma15_32'], decimals=3, space='buy', optimize=True)
    buy_cti_32 = DecimalParameter(-0.5, 0.5, default=buy_params['buy_cti_32'], decimals=2, space='buy', optimize=True)

    # Sell Params
    base_nb_candles_sell = IntParameter(2, 25, default=sell_params['base_nb_candles_sell'], space='sell', optimize=True)

    high_offset = DecimalParameter(0.99, 1.3, default=sell_params['high_offset'], space='sell', optimize=True)
    high_offset_2 = DecimalParameter(1.1, 1.6, default=sell_params['high_offset_2'], space='sell', optimize=True)
...
class NASOSv5_mod4_E0V1E_HO(eddies):
    # Buy hyperspace params:
    buy_params = {
...
# Removed by Mastaaa
#########################################
class NASOSv5_mod4_E0V1E_PD(eddies):
...
class NASOSv5_mod4_E0V1E_SL(eddies):
    sell_params = {
        "pHSL": -0.178,
        "pPF_1": 0.019,
        "pPF_2": 0.065,
...
class NASOSv5_mod4_E0V1E_TrailingBuyStrat(eddies):
