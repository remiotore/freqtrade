...
class PrintMoney_v1(IStrategy):
    minimal_roi = {
        # minutes since entry : target ROI
        "0":    0.12,   # 12% immediate
        "30":   0.05,   #  5% after 30m
        "60":   0.02,   #  2% after 1h
        "120":  0.01,   #  1% after 2h
        "240":  0.005,  # 0.5% after 4h
        "480":  0.0     #  0% (break even) after 8h
    }
    timeframe = '15m'
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        #     buy_btc = (
        #         # require a small pullback + early momentum
        #         (dataframe['rsi_slow'] < dataframe['rsi_slow'].shift(1))
        #         & (dataframe['rsi_fast'] < 45)              # allow higher fast-RSI
        #         & (dataframe['rsi'] > 32)                   # slightly lower main RSI floor
        #         & (dataframe['close'] < dataframe['sma_15'] * 0.99)  # within 1% of 15-SMA
        #         & (dataframe['cti'] < 0.8)                  # looser CTI threshold
        #     )
        #     conditions.append(buy_btc)
        #     dataframe.loc[buy_btc, 'enter_tag'] += 'buy_btc'
    ...
    def custom_exit(self, pair: str, trade: 'Trade', current_time: 'datetime', current_rate: float,
        ...
        # — ATR stop-loss —
        atr = talib.ATR(dataframe, timeperiod=14).iloc[-1]
