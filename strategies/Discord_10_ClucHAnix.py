...
from freqtrade.strategy import merge_informative_pair, DecimalParameter, stoploss_from_open, RealParameter, informative
...
class ClucHAnix(IStrategy):
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[
            (
                (dataframe['rocr_1h'] > self.rocr_1h.value) &
                (
                    (
                        (dataframe['lower'].shift() > 0) &
                        (dataframe['bbdelta'] > dataframe['ha_close'] * self.bbdelta_close.value) &
                        (dataframe['closedelta'] > dataframe['ha_close'] * self.closedelta_close.value) &
                        (dataframe['tail'] < dataframe['bbdelta'] * self.bbdelta_tail.value) &
                        (dataframe['ha_close'] < dataframe['lower'].shift()) &
