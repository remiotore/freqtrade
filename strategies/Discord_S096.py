...
class S096(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe['trend'] = dataframe['plus_di'] / dataframe['minus_di'] * 100
        dataframe['color'] = np.where(dataframe['plus_di'] > dataframe['minus_di'], 'green', np.where(dataframe['plus_di'] < dataframe['minus_di'], 'red', 'blue'))
        ...
        dataframe.loc[
            (
                (
                    #BULLISH & BEARISH
                    (
                        (dataframe['trend']>120) | (dataframe['trend']<80)
                    ) &
                    (
                        # Breakout: TEMA crosses over longer TEMA
                        (qtpylib.crossed_above(dataframe['tema5'], dataframe['tema14'])) &
                        (dataframe['adx'] > 25)
                    )
                    |
                    (
                        # Breakout: RSI plus momentum
                        (qtpylib.crossed_above(dataframe['rsi'], 60)) &
                        (dataframe['adx'] > 30)
                    )
                    |
                    (
                        # Breakout: crossing middle of Bollinger Bands
                    ...
                    |
                    (
                        # Breakout: Increasing momentum and positive price action
                    ...
                    |
                    (
                        # BTFD: Price Crosses into the Bollinger Bands from below
                        # Doesn't like BTC Dominance tracking sideways
                        # Enable by uncommenting the indicators
