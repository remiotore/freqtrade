...
class roger20(IStrategy):
    ...
    def custom_stoploss(self, pair: str, trade: 'Trade', current_profit: float, current_rate: float, **kwargs) -> float:
        ...
        trade_open_hours = (datetime.now(timezone.utc) - trade.open_date_utc).total_seconds() / 3600

        atr_stoploss = -last_candle['ATR'] / current_rate

        if trade_open_hours > 24:

            atr_stoploss *= 0.75  # Tighten by 25%
        elif trade_open_hours > 48:

            atr_stoploss *= 0.5  # Tighten by 50%

        if current_profit > 0.05:

            atr_stoploss *= 0.8  # Tighten by 20%
        elif current_profit > 0.1:

            atr_stoploss *= 0.6  # Tighten by 40%

        adjusted_stoploss = max(atr_stoploss, trade.stop_loss)
