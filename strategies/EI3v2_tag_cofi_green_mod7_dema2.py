...
class EI3v2_tag_cofi_green_mod7_dema2(IStrategy):
    INTERFACE_VERSION = 2
    ...
    def informative_pairs(self):
        ...
        if self.config['stake_currency'] in ['USDT','FDUSD','USDC','DAI','TUSD','PAX','USD','EUR','GBP']:
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if self.config['stake_currency'] in ['USDT','FDUSD']:
            btc_info_pair = f"BTC/{self.config['stake_currency']}"
        else:
            btc_info_pair = "BTC/USDT"
