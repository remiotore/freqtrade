...
class expired_v2(IStrategy):
    ...
    macd_ma_period = IntParameter(20, 48, default=buy_params["macd_ma_period"], space="buy", optimize=True, load=True)
    macd_signal_period = IntParameter(5, 15, default=buy_params["macd_signal_period"], space="buy", optimize=True, load=True)

    check_macd_position = BooleanParameter(default=buy_params["check_macd_position"], space="buy", optimize=True, load=True)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        triggers_long.append(
            qtpylib.crossed_above(
                dataframe[f"impulse_macd_{self.macd_ma_period.value}_{self.macd_signal_period.value}"],
                dataframe[f"impulse_macdsignal_{self.macd_ma_period.value}_{self.macd_signal_period.value}"],
            )
        )
        ...
        atr_ratio = 0.2
        guards_long.append(
            (dataframe[f"impulse_macd_{self.macd_ma_period.value}_{self.macd_signal_period.value}"] > atr_ratio * dataframe["atr"])
            | (dataframe[f"impulse_macd_{self.macd_ma_period.value}_{self.macd_signal_period.value}"] < -atr_ratio * dataframe["atr"])
        )
        guards_short.append(
            (dataframe[f"impulse_macd_{self.macd_ma_period.value}_{self.macd_signal_period.value}"] > atr_ratio * dataframe["atr"])
        ...
        if self.check_macd_position.value:
            ...
            guards_short.append(
                dataframe[f"impulse_macdsignal_{self.macd_ma_period.value}_{self.macd_signal_period.value}"] > 0
            )

        if triggers_long:
            dataframe.loc[reduce(lambda x, y: x | y, triggers_long) & reduce(lambda x, y: x & y, guards_long), "enter_long"] = 1

        if triggers_short:
            dataframe.loc[
                reduce(lambda x, y: x | y, triggers_short) & reduce(lambda x, y: x & y, guards_short), "enter_short"
            ] = 1
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if triggers_long:
            dataframe.loc[reduce(lambda x, y: x | y, triggers_long) & reduce(lambda x, y: x & y, guards_long), "exit_long"] = 1

        if triggers_short:
            dataframe.loc[
                reduce(lambda x, y: x | y, triggers_short) & reduce(lambda x, y: x & y, guards_short), "exit_short"
            ] = 1
    ...
    def custom_exit(
        ...
        if current_time - timedelta(minutes=90) < trade.open_date_utc and current_profit < -0.12:
            return "sudden_loss_exit"
